...
public class CmdResetPoint implements CommandExecutor
{
	@Override
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
	{
		...
		if (args.length >= 2 && (args[0].startsWith("n") || args[0].startsWith("N")))
		{
			...
			for (CapturePoint point : PvP.getCoreData().getPoints())
			{
				...
				{
					point.setOwner(null);
					sender.sendMessage(ChatColor.RED + "Reset point " + ChatColor.WHITE + point.getName());
					return true;
				}
			}
		...
		for (CapturePoint point : PvP.getCoreData().getPoints())
		{
			...
			{
				point.setOwner(null);
				sender.sendMessage(ChatColor.RED + "Reset point " + ChatColor.WHITE + point.getName() + ChatColor.RED + " at location " + ChatColor.WHITE + String.format("%s: %.0f,%.0f,%.0f", location.getWorld().getName(), location.getX(), location.getY(), location.getZ()));
				return true;
			}
		}
